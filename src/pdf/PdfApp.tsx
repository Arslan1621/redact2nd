...
const PDFRedactionTool = () => {
  ...
  // Handle mouse events for manual redaction and deletion
  const handleMouseDown = (e) => {
    if ((!redactionMode && !deleteMode) || !overlayCanvasRef.current) return;
    ...
    // If in delete mode, check if clicking on existing redaction
    if (deleteMode) {
      const clickedRedaction = redactions.find(r => 
        r.page === currentPage &&
        x >= r.x && x <= r.x + r.width &&
      ...
      if (clickedRedaction) {
        setRedactions(prev => prev.filter(r => r.id !== clickedRedaction.id));
        alert('Redaction removed successfully!');
        redrawRedactions();
        return;
      }
    }
    
    // Normal redaction mode
    if (redactionMode) {
  ...
  const handleMouseMove = (e) => {
    if (!isDrawing || !overlayCanvasRef.current || !redactionMode) return;
    ...
    const selectionX = Math.min(startPoint.current.x, x);
    const selectionY = Math.min(startPoint.current.y, y);
    const selectionWidth = Math.abs(x - startPoint.current.x);
    const selectionHeight = Math.abs(y - startPoint.current.y);
  ...
  const handleMouseUp = (e) => {
    ...
    // Only create redaction if area is large enough
    if (width > 10 && height > 10) {
      ...
      alert('Redaction area selected successfully!');
    }
    
    setIsDrawing(false);
    // Redraw to show final redactions
    setTimeout(() => redrawRedactions(), 10);
  };
  ...
  // Remove all redactions on current page
  const clearCurrentPageRedactions = () => {
    const confirmed = window.confirm(`Are you sure you want to remove all redactions from page ${currentPage}?`);
    if (confirmed) {
      setRedactions(prev => prev.filter(r => r.page !== currentPage));
      alert('All redactions removed from current page!');
      redrawRedactions();
    }
  };
  ...
  const toggleDeleteMode = () => {
    if (deleteMode) {
      ...
      setDeleteMode(true);
      setRedactionMode(false); // Exit redaction mode when entering delete mode
    }
  };
  ...
  const toggleRedactionMode = () => {
    if (redactionMode) {
      ...
      setDeleteMode(false); // Exit delete mode when entering redaction mode
    }
  };
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className="flex items-center px-3 py-1 bg-gray-200 text-gray-700 rounded hover:bg-gray-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                ...
                >
                  ...
                  {deleteMode ? 'Exit Delete Mode' : 'Delete Redactions'}
                </button>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                >
              ...
              <div className="max-w-4xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3">
                        <p className="text-sm text-orange-700 font-medium">
                          Delete mode is active. Click on any redaction to remove it.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="relative inline-block">
                  ...
                  <canvas
                    ...
                    style={{ 
                      cursor: redactionMode ? 'crosshair' : deleteMode ? 'pointer' : 'default' 
                    }}
